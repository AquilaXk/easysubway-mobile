...
const codes = ['INVALID_JOURNEY_REQUEST', 'STATION_NOT_FOUND', 'ROUTE_NOT_FOUND', 'ACCESSIBILITY_CONSTRAINT_UNSATISFIED', 'ROUTING_BUNDLE_UNAVAILABLE', 'ROUTING_BUNDLE_STALE', 'TIMETABLE_UNAVAILABLE', 'TIMETABLE_STALE', 'REALTIME_REQUIRED_UNAVAILABLE', 'ROUTING_IDENTITY_MISMATCH', 'ROUTE_SERVICE_UNAVAILABLE', 'JOURNEY_SEARCH_TIMEOUT', 'STATION_LINE_NOT_FOUND', 'TIMETABLE_NOT_COVERED', 'TIMETABLE_IDENTITY_MISMATCH', 'ROUTE_SESSION_REQUIRED', 'ROUTE_RATE_LIMITED', 'INVALID_JOURNEY_SESSION_REQUEST', 'ROUTE_SESSION_ATTESTATION_REJECTED', 'ROUTE_SESSION_ATTESTATION_UNAVAILABLE'];
...
components:
  ...
  schemas:
    ...
    JourneySearchRequest:
      type: object
      additionalProperties: false
      required: [requestId, originStationId, destinationStationId, departure, timePolicy, walkingPace, mobilityProfile, constraintMode, maxTransfers, alternativeCount]
      properties:
        requestId:
          type: string
        ...
        walkingPace:
          $ref: "#/components/schemas/WalkingPace"
        mobilityProfile:
...
function legacyTwoOperationFixture(root) {
...
function fixture(root) { return exactProducerFixture(root); }
...
test('builds closed producer-shaped IR including timetable selector variants and disposition bindings', () => { const root = mkdtempSync(join(tmpdir(), 'journey-generator-')); try { const value = fixture(root); const ir = validateJourneyV3ClientInputForTest(value); assert.equal(ir.operations.length, 3); assert.equal(ir.errorCatalog.length, 25); assert.equal(ir.schemas.JourneyDeparture.oneOf.length, 2); assert.equal(ir.schemas.JourneyLeg.oneOf.length, 4); assert.equal(ir.schemas.StationTimetableSelector.oneOf.length, 3); assert.equal(ir.errorDispositions.length, 25); assert.deepEqual(ir.errorCatalog.filter(({ operation }) => operation === 'searchStationTimetables').map(({ operation, httpStatus, code }) => [operation, httpStatus, code]), [['searchStationTimetables', 400, 'INVALID_JOURNEY_REQUEST'], ['searchStationTimetables', 404, 'STATION_LINE_NOT_FOUND'], ['searchStationTimetables', 404, 'TIMETABLE_NOT_COVERED'], ['searchStationTimetables', 503, 'TIMETABLE_UNAVAILABLE'], ['searchStationTimetables', 503, 'TIMETABLE_STALE'], ['searchStationTimetables', 503, 'TIMETABLE_IDENTITY_MISMATCH'], ['searchStationTimetables', 401, 'ROUTE_SESSION_REQUIRED'], ['searchStationTimetables', 429, 'ROUTE_RATE_LIMITED']]); assert.equal(Object.isFrozen(ir.errorDispositions[0]), true); assert.equal(ir.errorDispositions[0].code, 'INVALID_JOURNEY_REQUEST'); } finally { rmSync(root, { recursive: true, force: true }); } });
test('rejects the obsolete two-operation producer assumption', () => { const root = mkdtempSync(join(tmpdir(), 'journey-generator-')); try { assert.throws(() => validateJourneyV3ClientInputForTest(legacyTwoOperationFixture(root)), /OpenAPI version or path set is unsupported/); } finally { rmSync(root, { recursive: true, force: true }); } });
test('validates the exact locked four-resource producer bytes without sibling repository or network', () => { const root = mkdtempSync(join(tmpdir(), 'journey-generator-')); try { const ir = validateJourneyV3ClientInputForTest(exactProducerFixture(root)); assert.equal(ir.operations.length, 3); assert.equal(ir.errorCatalog.length, 25); assert.equal(ir.errorDispositions.length, 25); assert.equal(ir.sessionIntegrity.session.ttlSeconds, 600); } finally { rmSync(root, { recursive: true, force: true }); } });
