...
test('validates the exact locked four-resource producer bytes without sibling repository or network', () => { const root = mkdtempSync(join(tmpdir(), 'journey-generator-')); try { const ir = validateJourneyV3ClientInputForTest(exactProducerFixture(root)); assert.equal(ir.operations.length, 4); assert.equal(ir.errorCatalog.length, 41); assert.equal(ir.errorDispositions.length, 41); assert.equal(ir.sessionIntegrity.session.ttlSeconds, 600); } finally { rmSync(root, { recursive: true, force: true }); } });
