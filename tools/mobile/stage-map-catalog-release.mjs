...
export function parseJsonWithoutDuplicateKeys(bytes, label = "JSON") {
  let text;
  try { text = Buffer.isBuffer(bytes) ? new TextDecoder("utf-8", { fatal: true }).decode(bytes) : bytes; } catch { throw new Error(`${label} must be UTF-8 JSON`); }
  ...
  const value = () => { space(); if (text[index] === '"') return string(); if (text[index] === "{") { index += 1; space(); const output = Object.create(null); const seen = new Set(); if (text[index] === "}") { index += 1; return output; } while (true) { space(); const key = string(); if (seen.has(key)) throw new Error(`${label} has duplicate key ${key}`); seen.add(key); space(); if (text[index++] !== ":") throw new Error(`${label} has invalid JSON`); Object.defineProperty(output, key, { value: value(), enumerable: true, configurable: true, writable: true }); space(); if (text[index] === "}") { index += 1; return output; } if (text[index++] !== ",") throw new Error(`${label} has invalid JSON`); } } if (text[index] === "[") { index += 1; space(); const output = []; if (text[index] === "]") { index += 1; return output; } while (true) { output.push(value()); space(); if (text[index] === "]") { index += 1; return output; } if (text[index++] !== ",") throw new Error(`${label} has invalid JSON`); } } const primitive = /^(true|false|null|-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?)/.exec(text.slice(index)); if (!primitive) throw new Error(`${label} has invalid JSON`); index += primitive[0].length; return JSON.parse(primitive[0]); };
...
function validateArtifact(value, shape, index) {
  ...
  if (requirePath(manifest.path, `${label}.manifest.path`) !== shape.manifestPath || manifest.manifestVersion !== 1 || manifest.artifactKind !== shape.kind || typeof manifest[shape.id] !== "string" || !manifest[shape.id] || /\s/.test(manifest[shape.id][0]) || /\s/.test(manifest[shape.id].at(-1))) throw new Error(`${label}.manifest has invalid component semantics`);
