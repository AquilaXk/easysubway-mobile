...
export function validateWorkflowRunMetadata(runMetadata, lock) {
  const expected = validateGenericMobileConsumerBundleLock(lock);
  if (!runMetadata || Array.isArray(runMetadata) || typeof runMetadata !== "object" || !runMetadata.repository || Array.isArray(runMetadata.repository) || typeof runMetadata.repository !== "object" || runMetadata.id !== expected.publication.runId || runMetadata.workflow_id !== expected.publication.workflowId || runMetadata.run_attempt !== expected.publication.runAttempt || runMetadata.path !== expected.publication.workflowPath || runMetadata.event !== "workflow_dispatch" || runMetadata.status !== "completed" || runMetadata.conclusion !== "success" || runMetadata.head_branch !== "main" || runMetadata.head_sha !== expected.publication.headSha || runMetadata.repository.id !== expected.publication.repositoryId || runMetadata.repository.full_name !== expected.producer.repository) throw new Error("workflow run metadata does not match the immutable lock");
  return runMetadata;
}
