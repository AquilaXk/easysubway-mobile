...
test("schema snapshots are fixed to the Task 6 contract version and digest", async () => {
  ...
  assert.equal(sha256(lockSchema), "2004c2e35ceef2253cee79858fdf9c0ca5f91a290133688c3434d88c27186694");
  assert.equal(sha256(componentSchema), "c429eb092aa187ec15f7d74294efeaaf0c94681e5fc59c42ae9080b839473502");
});
...
test("trusted AAB entries require exact, unique, safe pinned bytes", () => {
  const identity = identityWithFixtureBytes();
  const valid = identity.packs.map((pack) => ({ name: `assets/datapacks/${pack.path}`, bytes: bytesFor(pack) }));
  ...
  for (const entries of [
    ...
    [{ name: "../assets/datapacks/core.sqlite.gz", bytes: Buffer.from("core") }, valid[1]],
    [{ ...valid[0], bytes: Buffer.from("wrong") }, valid[1]],
