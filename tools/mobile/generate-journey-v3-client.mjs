...
const fixedEnums = [
  ['JourneyContractVersion', ['JOURNEY_SEARCH_V3']], ['JourneyErrorContractVersion', ['JOURNEY_ERROR_V1']], ['JourneySessionScope', ['journey:v3']], ['JourneyDepartureMode', ['NOW', 'SCHEDULED']], ['JourneyStatus', ['FOUND']], ['JourneyPlanSource', ['SERVER_TIMETABLE_RAPTOR']], ['JourneyTimeSource', ['TIMETABLE', 'REALTIME']], ['JourneyAccessibilityResult', ['VERIFIED']], ['JourneyLegType', ['ENTRY', 'RIDE', 'TRANSFER', 'EXIT']], ['JourneyOperation', ['issueJourneySession', 'searchJourneys']],
];
...
export function renderJourneyV3ResponseModelsForTest(options) { validate({ ...options, enforceTrackedLock: false }); return renderResponseModels(); }
...
function renderContract(lock) { return `// Test-only Journey V3 contract barrel; production output remains closed.\nexport 'journey_v3_enums.dart';\nexport 'journey_v3_error.dart';\nexport 'journey_v3_models.dart';\nexport 'journey_v3_validation.dart';\nconst String journeyV3ProducerRepository = '${lock.producer.repository}';\nconst String journeyV3ProducerSha = '${lock.producer.gitSha}';\nconst String journeyV3ManifestDigest = '${lock.artifact.manifestDigest}';\nconst String journeyV3PayloadSha256 = '${lock.payload.sha256}';\nconst String journeyV3PublicationReceiptSha256 = '${lock.publicationReceiptSha256}';\n`; }
function renderDartContract(lock) { let source = renderContract(lock); for (const value of [lock.producer.repository, lock.producer.gitSha, lock.artifact.manifestDigest, lock.payload.sha256, lock.publicationReceiptSha256]) source = source.replaceAll(`'${value}'`, dartLiteral(value)); return source; }
