...
test("rejects an AAB whose bundle config lacks 16KB alignment", async () => {
  ...
  const bundletool = path.join(fixture, "bundletool");
  const evidence = path.join(fixture, "evidence");
  await writeFile(aab, "fixture");
  await writeFile(bundletool, "#!/usr/bin/env bash\nprintf '%s\\n' '{\\\"compression\\\": {}}'\n");
  await chmod(bundletool, 0o755);

  await assert.rejects(
    execFileAsync(gate, ["--aab", aab, "--artifact-dir", evidence, "--bundletool", bundletool]),
    (error) => error.code === 1,
  );
