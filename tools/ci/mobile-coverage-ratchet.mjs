...
const lcovTagSubset = (bytes) => { const lines = utf8(bytes, "normalized LCOV").split("\n"); return TAGS.filter((tag) => lines.some((line) => tag === "end_of_record" ? line === tag : line.startsWith(`${tag}:`))); };
...
function validateArtifactNestedEvidence({ raw, normalized, inventory, result, policy, repositoryRoot, readFile }) {
  const identityKeys = ["event", "eventMode", "baseSha", "headSha", "mergeBaseSha", "testedMergeSha", "range"];
  exactKeys(inventory.identity, identityKeys, "inventory identity"); exactKeys(result.identity, identityKeys, "result identity");
  exactKeys(inventory.producer, ["policySha256", "filterSha256", "ratchetSha256", "lcovTagSubset"], "inventory producer");
  exactKeys(result.producer, ["policySha256", "baselineSha256", "filterSha256", "ratchetSha256", "rawLcovSha256", "normalizedLcovSha256", "sourceInventorySha256", "lcovTagSubset"], "result producer");
  exactKeys(inventory.summary, ["sources", "included", "excluded", "lcovPresent", "lcovMissing", "executableLines", "coveredLines", "lineBasisPoints"], "inventory summary");
  exactKeys(result.coverage, ["executableLines", "coveredLines", "uncoveredLines", "lineBasisPoints", "lcovMissingSources"], "coverage");
  exactKeys(result.changedLines, ["state", "entries", "executableLines", "coveredLines", "lineBasisPoints"], "changed lines");
  exactKeys(result.artifacts, ["directory", "name", "files"], "artifacts");
  if (!Array.isArray(inventory.sources) || !Array.isArray(result.changedLines.entries) || !Array.isArray(result.criticalBoundaries) || !Array.isArray(result.exclusions) || JSON.stringify(result.identity) !== JSON.stringify(inventory.identity)) fail("artifact nested evidence shape mismatch");
  if (inventory.schemaVersion !== 1 || inventory.artifactKind !== "mobile-coverage-source-inventory-v1" || inventory.repository !== policy.repository || result.schemaVersion !== 1 || result.artifactKind !== "mobile-coverage-ratchet-result-v1" || result.repository !== policy.repository) fail("artifact top-level identity is invalid");
  const identity = result.identity;
  if (!SHA.test(identity.baseSha) || !SHA.test(identity.headSha) || !SHA.test(identity.mergeBaseSha) || !SHA.test(identity.testedMergeSha) || ![["pull_request", "PULL_REQUEST"], ["push", "PUSH_MAIN"], ["workflow_dispatch", "MANUAL_FULL"]].some(([event, mode]) => identity.event === event && identity.eventMode === mode)) fail("artifact identity is invalid");
  if (JSON.stringify(inventory.producer) !== JSON.stringify({ policySha256: result.producer.policySha256, filterSha256: result.producer.filterSha256, ratchetSha256: result.producer.ratchetSha256, lcovTagSubset: result.producer.lcovTagSubset })) fail("artifact inventory producer does not match current producer");
  const gitApi = defaultGit(repositoryRoot); const ancestor = (base, target) => { try { gitApi.text(["merge-base", "--is-ancestor", base, target]); return true; } catch { return false; } }; if (gitApi.text(["rev-parse", "HEAD"]) !== identity.testedMergeSha) fail("artifact tested merge does not equal current HEAD");
  if (identity.eventMode === "PULL_REQUEST") { const mergeBases = gitApi.text(["merge-base", "--all", identity.baseSha, identity.headSha]).split("\n").filter(Boolean); if (identity.testedMergeSha === identity.headSha || mergeBases.length !== 1 || mergeBases[0] !== identity.mergeBaseSha || identity.range !== `${identity.mergeBaseSha}..${identity.headSha}` || !ancestor(identity.mergeBaseSha, identity.baseSha) || !ancestor(identity.mergeBaseSha, identity.headSha) || !ancestor(identity.baseSha, identity.testedMergeSha) || !ancestor(identity.headSha, identity.testedMergeSha)) fail("artifact pull request identity relation is invalid"); }
  else if (identity.eventMode === "PUSH_MAIN") { if (identity.baseSha === identity.headSha || identity.headSha !== identity.testedMergeSha || identity.mergeBaseSha !== identity.baseSha || identity.range !== `${identity.baseSha}..${identity.headSha}` || !ancestor(identity.baseSha, identity.headSha)) fail("artifact push identity relation is invalid"); }
  else if (identity.baseSha !== identity.headSha || identity.headSha !== identity.mergeBaseSha || identity.mergeBaseSha !== identity.testedMergeSha || identity.range !== null) fail("artifact manual identity relation is invalid");
  const expectedNormalized = Buffer.from(normalizeLcov(utf8(raw, "raw LCOV"), { policy, repositoryRoot, policySha256: result.producer.policySha256 }).content); const tagSubset = lcovTagSubset(normalized); if (!expectedNormalized.equals(normalized) || !tagSubset.length || JSON.stringify(result.producer.lcovTagSubset) !== JSON.stringify(tagSubset)) fail("artifact normalized LCOV/tag subset is not exact");
  const coverage = parseNormalizedLcov(normalized); const rawCoverage = parseNormalizedLcov(raw); const changedPolicy = parsedJson(readFile(path.join(repositoryRoot, "tools/ci/mobile-changed-path-policy.json")), "#50 changed path policy"); const testedTree = treeSources(identity.testedMergeSha, gitApi);
  const sources = [...testedTree.keys()].map((file) => {
    const blob = testedTree.get(file); const sourceBytes = gitApi.bytes(["cat-file", "blob", blob]); const excluded = policy.exclusions.find((entry) => entry.path === file); const lines = coverage.get(file); const executable = lines?.size ?? null; const covered = lines ? [...lines.values()].filter((hits) => hits > 0).length : null;
    return { path: file, sourceSha256: hash(sourceBytes), kind: "PRODUCTION_DART", owners: ownersFor(file, changedPolicy), criticalBoundaries: boundariesFor(file, policy), lcovPresent: Boolean(lines), absenceDisposition: null, executableLines: executable, coveredLines: covered, uncoveredLines: lines ? executable - covered : null, exclusion: excluded ? { excluded: true, reason: excluded.id } : { excluded: false, reason: null } };
  });
  const included = sources.filter((source) => !source.exclusion.excluded && source.lcovPresent); const totals = (items) => ({ executableLines: items.reduce((sum, item) => sum + item.executableLines, 0), coveredLines: items.reduce((sum, item) => sum + item.coveredLines, 0) }); const all = totals(included);
  const summary = { sources: sources.length, included: sources.filter((source) => !source.exclusion.excluded).length, excluded: sources.filter((source) => source.exclusion.excluded).length, lcovPresent: sources.filter((source) => source.lcovPresent).length, lcovMissing: sources.filter((source) => !source.lcovPresent).length, executableLines: all.executableLines, coveredLines: all.coveredLines, lineBasisPoints: bp(all.coveredLines, all.executableLines) };
  const changedLines = identity.eventMode === "MANUAL_FULL" ? { state: "NOT_APPLICABLE_MANUAL_FULL", entries: [], executableLines: 0, coveredLines: 0, lineBasisPoints: null } : changedExecutableLines(identity.range, identity.headSha, identity.testedMergeSha, coverage, gitApi);
  const boundaries = BOUNDARIES.map((id) => { const total = totals(included.filter((source) => source.criticalBoundaries.includes(id))); return { id, executableLines: total.executableLines, coveredLines: total.coveredLines, uncoveredLines: total.executableLines - total.coveredLines, lineBasisPoints: bp(total.coveredLines, total.executableLines), lcovMissingSources: sources.filter((source) => !source.lcovPresent && source.criticalBoundaries.includes(id)).map((source) => source.path) }; });
  const exclusions = policy.exclusions.map((entry) => { const lines = rawCoverage.get(entry.path); if (!lines || coverage.has(entry.path)) fail("artifact exclusion/filter evidence is invalid"); const executableLines = lines.size; const coveredLines = [...lines.values()].filter((hits) => hits > 0).length; return { id: entry.id, path: entry.path, reason: entry.id, executableLines, coveredLines }; });
  if (JSON.stringify(inventory.sources) !== JSON.stringify(sources) || JSON.stringify(inventory.summary) !== JSON.stringify(summary) || JSON.stringify(result.coverage) !== JSON.stringify({ executableLines: all.executableLines, coveredLines: all.coveredLines, uncoveredLines: all.executableLines - all.coveredLines, lineBasisPoints: bp(all.coveredLines, all.executableLines), lcovMissingSources: sources.filter((source) => !source.lcovPresent).map((source) => source.path) }) || JSON.stringify(result.changedLines) !== JSON.stringify(changedLines) || JSON.stringify(result.criticalBoundaries) !== JSON.stringify(boundaries) || JSON.stringify(result.exclusions) !== JSON.stringify(exclusions) || JSON.stringify(result.artifacts) !== JSON.stringify({ directory: policy.artifacts.directory, name: policy.artifacts.name, files: ARTIFACT_FILES })) fail("artifact nested evidence does not recompute exactly");
}

export function verifyArtifactDirectory(directory, { repositoryRoot = process.cwd(), readFile = readFileSync } = {}) {
  safeDirectory(directory); const names = readdirSync(directory).sort(compare); if (JSON.stringify(names) !== JSON.stringify([...ARTIFACT_FILES].sort(compare))) fail("artifact file set changed"); const raw = safeArtifactFile(directory, ARTIFACT_FILES[0]); const normalized = safeArtifactFile(directory, ARTIFACT_FILES[1]); const inventoryBytes = safeArtifactFile(directory, ARTIFACT_FILES[2]); const resultBytes = safeArtifactFile(directory, ARTIFACT_FILES[3]); const summary = safeArtifactFile(directory, ARTIFACT_FILES[4]); const inventory = canonical(inventoryBytes, "source inventory"); const result = canonical(resultBytes, "result"); exactKeys(inventory, INVENTORY_KEYS, "source inventory"); exactKeys(result, RESULT_KEYS, "result"); if (result.phase !== "DISCOVERY_REMOTE_RED" || result.outcome !== "DISCOVERY_REMOTE_RED" || JSON.stringify(result.reasons) !== JSON.stringify(["BASELINE_UNREVIEWED"]) || result.producer?.rawLcovSha256 !== hash(raw) || result.producer?.normalizedLcovSha256 !== hash(normalized) || result.producer?.sourceInventorySha256 !== hash(inventoryBytes) || JSON.stringify(result.identity) !== JSON.stringify(inventory.identity) || JSON.stringify(result.producer.lcovTagSubset) !== JSON.stringify(inventory.producer?.lcovTagSubset) || !result.producer.lcovTagSubset.every((tag) => TAGS.includes(tag)) || utf8(summary, "summary") !== summaryFor(result)) fail("artifact result cross-schema identity mismatch"); const policyBytes = readFile(path.join(repositoryRoot, "tools/ci/mobile-coverage-policy.json")); const baselineBytes = readFile(path.join(repositoryRoot, "tools/ci/mobile-coverage-baseline.json")); const filterBytes = readFile(path.join(repositoryRoot, "tools/ci/filter-mobile-lcov.mjs")); const ratchetBytes = readFile(fileURLToPath(import.meta.url)); const policy = parsePolicyBytes(policyBytes); parseBaselineBytes(baselineBytes); if (result.producer.policySha256 !== hash(policyBytes) || result.producer.baselineSha256 !== hash(baselineBytes) || result.producer.filterSha256 !== hash(filterBytes) || result.producer.ratchetSha256 !== hash(ratchetBytes) || !DIGEST.test(result.producer.rawLcovSha256) || !DIGEST.test(result.producer.normalizedLcovSha256) || !DIGEST.test(result.producer.sourceInventorySha256)) fail("artifact producer digest mismatch"); validateArtifactNestedEvidence({ raw, normalized, inventory, result, policy, repositoryRoot, readFile }); return result;
}
