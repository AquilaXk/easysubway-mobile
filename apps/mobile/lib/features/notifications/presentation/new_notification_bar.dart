...
///
/// 운행 공지 projection([hasDisruption])이 있거나 알림함에 새 항목
/// ([hasNotificationItems])이 있으면 전체 너비 바로 안내한다. 둘 다 없으면 빈 공간
/// 없이 사라진다. 바를 누르면 알림함으로 이동한다(단일 탭 경로).
class NewNotificationBar extends StatelessWidget {
  const NewNotificationBar({
    required this.disruptionChanges,
    required this.hasDisruption,
    required this.hasNotificationItems,
    required this.onOpenInbox,
    super.key,
  });

  final Listenable? disruptionChanges;
  final bool Function()? hasDisruption;
  final bool hasNotificationItems;
  final VoidCallback onOpenInbox;

  @override
  Widget build(BuildContext context) {
    final changes = disruptionChanges;
    if (changes == null) {
      return _buildBar(
        visible: (hasDisruption?.call() ?? false) || hasNotificationItems,
      );
    }
    return ListenableBuilder(
      ...
      builder: (context, _) {
        final visible =
            (hasDisruption?.call() ?? false) || hasNotificationItems;
