...
// 라벨 솔버는 #2068 SVG 충실도(2026-07-26 오너 결정) 이후 **구조화 노선도 모드
// 전용**이다 — 바탕층 모드는 오너 SVG 라벨이 .vec에 구워져 앱이 솔버를 아예
// 호출하지 않는다(basemap_labels_are_baked_gate_test.dart가 그 결정을 지킨다).
// 그래서 옛 basemap 전용 경로(오너 앵커 고정 배치·동명 폴백 억제·노드/표장
// 장애물·권역별 앵커 거리 게이트)를 재던 테스트들은 대상 코드와 함께 제거했다.

// 결정적 실측: 글자 수 × 9px, 높이 13px (bold·fontSize 무관 — 이 파일 전
// 테스트가 fontSize=13(kRouteMapDesignLabelFontPx)만 쓰므로 fontSize 인자는
// 시그니처만 맞추고 무시해도 안전하다).
Size _measureLabel(
  String text, {
  required bool bold,
...
void main() {
  ...
  test('솔버 호출은 debug 카운터로 관측된다 (바탕층 미호출 게이트의 신호원)', () {
    // basemap_labels_are_baked_gate_test.dart가 이 카운터로 "바탕층 모드에서
    // 솔버가 안 돈다"를 단언한다 — 카운터가 실제로 증가하는지 여기서 고정해,
    // 계측이 죽어 게이트가 조용히 무력화되는 것을 막는다.
    final before = debugRouteMapLabelSolverInvocationCount;
    _solve(_gridMap(count: 6));
    expect(debugRouteMapLabelSolverInvocationCount, before + 1);
  });
